...
namespace MH.ReCaptcha
{
    internal static class ReCaptchaGenerator
    {
        ...
        public static IHtmlContent ReCaptchaV3(string siteKey, string action, string language, string callBack)
        {
            ...
            content.AppendHtml("var x = document.getElementsByName('g-recaptcha-response');");
            content.AppendHtml("var i;");
            content.AppendHtml("for (i = 0; i < x.length; i++) {if (x[i].type == 'hidden') {x[i].value = token;}}");
            content.AppendHtml("});");
            content.AppendHtml("}");
